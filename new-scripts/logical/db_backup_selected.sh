...
#
# Backup mode (derived from TARGET_DBS, see below):
#   TARGET_DBS=ALL        -> backs up every non-system database
#   TARGET_DBS=<list>     -> backs up only the databases in the list
#                            (JSON array or CSV, as selected in the GUI)
#
...
#   the API passes positional args $1-$8; this script uses $1, $4, $5, $7 and $8.
#
# $1  RESULT_ID  - BJR01.CentralResultId this run belongs to; stored in
#                  BLS01.S01F02 on every status row this script inserts.
# $4  TARGET_DBS - BJC03.C03F04; "ALL" or the selected database list
#                  (JSON array or CSV) chosen in the GUI for this job.
...
#                  used to namespace the output filename so a file can be
#                  matched back to its backup job by eye at restore time.
...
BACKUP_FOLDER="Cloud-Live-DB-Tirupati"   # appended to BASE_DIR to form the actual backup path
PARALLEL=3
...
BP_SHRINK_SIZE=8589934592    # 8 GB
BP_NORMAL_SIZE=17179869184   # 16 GB
...
[ -n "${TARGET_DBS:-}" ] || missing+=("TARGET_DBS (\$4)")
...
if [ "${#missing[@]}" -gt 0 ]; then
  ...
  echo "Usage: $0 expects positional args \$1=RESULT_ID \$4=TARGET_DBS \$5=BASE_DIR \$7=STATUS_DB \$8=CONFIG_ID" >&2
  exit 1
fi

# --- Backup mode derived from TARGET_DBS ($4) ---
...
dump_one() {
  ...
  local SQL_FILE="$DB_DIR/job${CONFIG_ID}_${DB}_${TS}.sql"
  local ARCHIVE_FILE="$DB_DIR/job${CONFIG_ID}_${DB}_${TS}.tar.gz"
  local DUMP_ERR="$DB_DIR/.dump_err_$$"

  mkdir -p "$DB_DIR"
  log_info "[$DB] start -> ${DB}/job${CONFIG_ID}_${DB}_${TS}.tar.gz"
  ...
  sync "$SQL_FILE"
  tar -czf "$ARCHIVE_FILE" -C "$DB_DIR" "job${CONFIG_ID}_${DB}_${TS}.sql"
  rm -f "$SQL_FILE"
  ...
  mysql_run "UPDATE ${STATUS_DB}.BLS01 SET S01F07='SUCCESS', S01C07=NOW(), S01F09='$ARCHIVE_FILE', S01F10=$size_bytes, S01F11=$size_bytes, S01F12='$checksum', S01F13=0 WHERE S01F02=$RESULT_ID AND S01F05='$DB' AND S01F07='RUNNING';"
...
if [ "$BACKUP_MODE" = "ALL" ]; then
  ...
  # TARGET_DBS ($4 / BJC03.C03F04) holds the GUI-selected list, as either a
  # JSON array (["db1","db2"]) or CSV (db1,db2). Normalize either form into
  # the same newline-separated format that the ALL branch produces above.
  case "$TARGET_DBS" in
    \[*\])
      DATABASES=$(echo "$TARGET_DBS" | tr -d '[]"' | tr ',' '\n')
      ;;
    *)
      DATABASES=$(echo "$TARGET_DBS" | tr ',' '\n')
      ;;
  esac
  DATABASES=$(echo "$DATABASES" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | grep -v '^$')

  [ -n "$DATABASES" ] || { log_error "TARGET_DBS (\$4) resolved to an empty database list: '$TARGET_DBS'"; exit 1; }
else
...
export -f dump_one _log log_info log_warn log_error mysql_run
