...
BACKUP_FOLDER="TEMP"   # appended to BASE_DIR to form the actual backup path
...
if [ "$BACKUP_MODE" = "ALL" ]; then
  DATABASES=$(mysql_val "SHOW DATABASES" | grep -Ev "^(information_schema|performance_schema|mysql|sys|DBBRD_STATUS)$")
...
# Pre-Backup Buffer Pool Cleanup
