#!/usr/bin/env bash
#
# Usage: ./binlog_collect.sh   (no arguments)
#
set -euo pipefail
...
# Backup lock — LOCAL. MUST match LOCK_DIR + BACKUP_BASE basename in backup.sh.
...
# Must run BEFORE the anchor lookup, which finds nothing on an unreachable mount.
############################
...
cleanup_on_error() {
  ...
  # Only if WE created it this run, or the resume point is reset.
...
#
# Selected by FILENAME, not mtime: SMB mtime is attribute-cached and skews.
#
# Date and time sorted as SEPARATE keys; a bare date counts as time 000000.
############################
...
# Under logs/<anchor>/collect/, so binlog/<anchor>/ stays pure data. Appended.
...
# Must stay AFTER STATE_FILE is defined, or cleanup deletes a file it did not make.
############################
...
# EXACTLY six digits: a bare glob also matches binlog.index and binlog.sha256,
# whose "sequence" is read as a variable name inside $(( )) and aborts on set -u.
...
for BINLOG_FILE in "${BINLOG_FILES[@]}"; do
  ...
  # cp without -p: CIFS cannot preserve ownership, so -p fails on good copies.
  ...
  # apply_binlog.sh re-verifies against this. Bare filenames, for any directory.
...
while read -r f; do
  # Strip leading zeros: bash arithmetic reads 000042 as octal. Empty = 000000.
