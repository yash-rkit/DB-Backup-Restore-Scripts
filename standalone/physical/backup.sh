...
# LOCAL staging, never a CIFS path; ~1.5x the data size, emptied every run.
BACKUP_BASE="/Data/dbvault-stage"

# Staging space required, as a % of the data size (150 = 1.5x).
LOCAL_SPACE_PCT=150
...
# The CIFS MOUNT POINT itself — usually a PARENT of the above; set both the
# same if the share is mounted directly.
SMB_MOUNT_POINT="/livestorage"

# Backup lock — LOCAL to this VM, polled by binlog_collect.sh. Never on a mount.
...
# Mount check before anything touches the share; duplicated by Check 8.
...
# Date-prefixed; binlog_collect.sh polls this exact path. Always removed.
...
# Publish this run's logs to SECONDARY/logs/<NAME>/. Best-effort, both paths.
PUBLISHED_LOGS=0
publish_logs() {
...
cleanup_on_error() {
  ...
  # A truncated manifest is worse than none.
...
# Base name for every artifact: the run date, plus a time on a same-day rerun.
NAME="${TODAY}"
...
# Written to local staging during the run, then MOVED to SECONDARY/logs/<NAME>/.
...
# Same-day collision check against SECONDARY — scratch is wiped every run.
...
# Counted, not grep -q: both phases append to the same log.
...
# Repointed at the permanent location so sha256sum -c resolves from anywhere.
...
# binlog_collect.sh anchors on this and reads only the share — must land there.
...
# Best-effort: the archive is complete, so a binlog problem must not fail the run.
############################
