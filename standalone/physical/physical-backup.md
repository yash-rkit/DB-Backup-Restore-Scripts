...
| Script | Role | Run by |
|---|---|---|
| `backup.sh` | full XtraBackup → prepare → compress → checksum → publish | crontab, daily |
...
`CMC27` tokens. Everything is configured in each script's `CONFIGURATION` block
and all state lives on disk and in the log files. The per-database counterpart is
[../logical/logical-backup.md](../logical/logical-backup.md).

## 1. The two facts everything follows from

Almost every non-obvious decision in these five scripts traces back to one of
two constraints. Read this before changing anything.
...
corrupt, and you find out at restore time. Hence: stage on local disk, prepare
...
passes, `ls` passes — and writes land on the root filesystem and fill it. Every
script therefore calls `mountpoint -q` on the **exact mount point**, never a `-d`
test. That is also why `SMB_MOUNT_POINT` is configured separately from
`SECONDARY_STORAGE_DIR`: `mountpoint -q` only returns true for the exact mount
point, so aiming it at a subdirectory would make every run fail.
...
**mtime is not trustworthy.** It comes from the server clock and is
attribute-cached, so `binlog_collect.sh` picks its anchor by **filename**, never
by newest mtime — a clock skew would silently reset collection to an older
...
Recovery is by binlog **file + position** only. Everything in `apply_binlog.sh`
follows from this:
...
- **Apply is one-shot.** If it fails partway it cannot be resumed: restarting
  from the failed file duplicates data, starting from the next file loses data.
  The only correct response is to restore the full backup again and retry.
- **Therefore** the script halts on the *first* error rather than continuing, and
  *hard-refuses* a second apply rather than warning.
...
would be belt-and-braces. It is off, so the script *is* the safety net.
`--skip-gtids` is deliberately not passed: it would be a no-op implying a
protection this setup does not have.

## 2. Published layout

Everything permanent lives on the share. Local staging is emptied after every run.
...
├── binlog/20260809/            binlogs belonging to that backup
│   ├── binlog.000007
...
Three deliberate properties: **restorable artifacts stay flat at the top level**,
so `*.tar.gz` and `*_binlog_info` globs stay simple; **`logs/` is a separate
subtree**, so retention can prune logs without touching archives; and
**`binlog/<date>/` is pure data**, no logs mixed in. Filenames inside
`logs/<date>/` carry no date prefix — the folder already does.

Two pieces of state are deliberately **not** on the share:

| Path | Why |
|---|---|
| `/var/lock/dbvault/` | a lock only means anything to processes on this host, and on a network mount a dropped mount makes `[[ -f ]]` read as "no backup running" — the interlock would fail exactly when it matters. `/var/lock` is tmpfs, so a reboot clears a lock left by a hard kill |
| `/var/lib/dbvault/` | the restore marker records what **this server** did, not a property of the backup. Two servers restoring the same archive would overwrite each other's marker and the double-apply guard would read the wrong state. `/var/lib`, not `/var/lock`, so it survives a reboot |
...
There is no job prefix, because one destination folder holds exactly one backup
job. **Two backup jobs must therefore never share a destination folder** — without
a prefix their same-day archives collide on identical names.
...
it collects against, and its name becomes the `binlog/<name>/` and `logs/<name>/`
folder. Selection sorts on **date and time as separate keys**, not a plain lexical
sort:
...
There is no fixed lookback window, so a backup older than *N* days is still found
rather than the collector reporting "nothing to collect".
...
that point go silently wrong on a dropped mount: the same-day collision test reads
`<share>/<date>.tar.gz` and on an unmounted path reads false, so a rerun reuses a
name already taken on the NAS; and `mkdir -p` would **create** the share tree
locally under the bare mount point, filling the root filesystem.
...
**Stage 7 re-asserts the mount** before copying. Pre-flight may have passed hours
ago on a large dataset, and a share that dropped since would look like an empty
local directory: the copy would "succeed" onto the root filesystem and even pass
its own checksum re-read, publishing a backup that is not on the NAS at all.

**Stage 8 is deliberately best-effort.** By then the archive is complete and
verified, so a binlog problem must not fail the run. It exists only to start PITR
coverage immediately rather than waiting up to 15 minutes for the next collector.

### The manifest

A plain-text sidecar written **outside** the tarball on purpose: `restore_full.sh`
needs the checksum, the source MySQL version and the prepared flag *before* it
...
`recovery_method=file_position` is recorded deliberately. Whoever reads this in
two years needs the one-shot constraint stated, not inferred from a missing GTID
field.

## 5. `binlog_collect.sh`

The archive alone restores you to the instant the backup ran. Binlogs carry you
forward, and that only works if collection is unbroken.

Each run: **check the share is reachable** (unreachable → exit 1 loudly) →
**check the lock** (backup running → exit 0, §6) → **find the anchor** (§3) →
**determine the start point** (`last_copied_binlog` if present, else the anchor
position) → **`FLUSH BINARY LOGS`** so the active binlog becomes closed and safe
to copy → **copy** every binlog from the start point on, skipping the currently
active one → **check sequence continuity** across everything archived so far.
...
collecting nothing and PITR coverage disappears unnoticed. The reachability check
therefore runs *before* the anchor lookup and fails loudly.
...
| `sha256sum` recorded | lets `apply_binlog.sh` detect a binlog that rotted on the share between collection and recovery — the window where nothing else is watching |

A binlog failing any of these is **deleted rather than archived**, and the resume
state only advances after a fully validated copy. `binlog.sha256` is append-only
and stores **bare** filenames, so verification works from wherever the binlogs are
staged later.

### Purged binlogs, gaps, and the exit code

If the start binlog is gone from disk (MySQL expired it) the collector falls back
to the earliest available one and logs a `CRITICAL` warning naming the gap.
...
duration**. The sequence check catches the other shape of the same problem — a
hole in the middle of the archived run — and has to be caught here, while the
missing file may still be on the source server.

Any error exits **non-zero**, because the PITR chain is not intact. An earlier
version logged a warning and exited 0: cron saw a green run every 15 minutes
while recovery coverage was quietly broken, and a monitored job that cannot fail
is worse than no monitoring. This is a *partial-success* exit — binlogs that
copied and validated stay archived and the state file still points at the last
good one, so the next run resumes correctly rather than starting over.
...
`backup.sh` writes a date-stamped lock at the start and removes it at the end, on
success *and* on failure via the trap. `binlog_collect.sh` checks for it and exits
0 if present.
...
that exact path from `LOCK_DIR` + `BACKUP_BASE_NAME`, so **`BACKUP_BASE_NAME` must
be updated if `BACKUP_BASE` changes** — a mismatch is silent and means the
collector runs *during* backups.

A lock older than `LOCK_STALE_SECONDS` (default 6h) means the backup probably
crashed without cleanup; the collector warns and proceeds rather than blocking
collection forever. Both scripts must run as the **same user**, or the interlock
does not work.
...
The archive is **never staged locally** — it is read directly from the share. That
keeps the local requirement to just the extracted datadir, at two costs worth
knowing. The archive is read over the network **twice**, once to verify and once
to extract; verification is not skipped to save that read, because extracting an
unverified archive over a wiped datadir is how a corrupt backup becomes a corrupt
database. And the network sits in the middle of the **destructive** phase: if the
share stalls during extraction the datadir is already wiped and left partially
populated, and the fix is to wait for the share and re-run from the start with the
database down until then. Verification happens *before* the wipe, so a corrupt or
unreachable archive still costs nothing.
...
for backups predating manifests. If neither exists the script **refuses** — that
is the whole point.

**The version comparison is informational only.** The classic Recovery Services
Vault trap: a VM restored from an image often carries an *older* MySQL than the
...
after partial writes. It warns rather than aborts because the comparison is a
crude string match, and a false positive that blocked the restore would leave you
unable to recover at all. If MySQL will not start at the end, check this first.
...
4. **Validate the extracted datadir** — core artifacts present, and
   ...
   disk**. Re-hashing the tarball here would prove only that the file just read is
   the file just read.

Then MySQL is started, waited on for up to 120s, and the restore marker written.
A datadir that fails validation is deliberately **not** auto-started: mysqld on a
partial datadir writes recovery state that makes a clean retry harder.

**When this finishes the database is UP but not current** — it holds data as of
the backup, not as of the failure. Managing application access during that window
...
Read §1, Fact 2 before changing anything here.

### The two guards that matter

**Check 3 — a restore marker must exist.** Applying binlogs to a database that was
not restored from *this exact* backup applies transactions against the wrong
...
path is on CIFS and the share stalls, the pipe breaks mid-transaction-stream — the
exact failure a one-shot apply cannot absorb.

`rsync` is preferred but **optional**, with a `cp` fallback: a missing convenience
tool must not block recovery during an incident. The checksum verification
afterwards is what establishes integrity, and it runs identically either way.
Staged binlogs are verified against `binlog.sha256` from collection time; a
mismatch is a hard stop, since applying a corrupted binlog injects corrupt data.
...
### Dry run, apply, and failure

The dry run decodes exactly what *would* be applied into
`<LOCAL_STAGE>/<id>_binlog_preview.sql`, touches nothing, and greps the result for
`DROP`/`TRUNCATE`/`ALTER`. With a one-shot apply this is the cheapest risk
reduction available, and the only chance to notice that the incident you are
recovering *from* is itself sitting in the binlogs about to be replayed.
...
recovery — keep applications away, inspect the cause, roll back with
`restore_full.sh`, fix, retry. Rolling back re-reads the full archive from the
share, so budget the same transfer time as the original restore.
...
Afterwards: verify row counts on the busiest tables and run application smoke
tests, then **take a fresh full backup immediately**. The binlog chain restarts at
this recovery point; until a new full backup exists the server has no usable
recovery baseline.
...
It contains **no restore logic of its own, deliberately**. The previous generation
duplicated the logic in a third place and the copies drifted — a different MySQL
user, a different base path — so the wrapper silently stopped matching the scripts
it was meant to mirror. Every fix would otherwise have to be made three times, and
one copy always gets missed.

**`--dry-run` is rejected, not forwarded.** Forwarding it would run the destructive
Phase 1 in full and only then preview Phase 2 — the opposite of what anyone typing
"dry run" expects, and unrecoverable by the time they find out.

**Its lock is a different file** (`restore_wrapper.lock`) from the `restore.lock`
its children take. Using the same one would deadlock: each child opens its own
file description and would block on the lock the wrapper already holds.

For anything non-routine, run the phases by hand. The staged flow gives you a
checkpoint between "data restored" and "binlogs applied", the last point at which
...
Each exists because violating it can lose data.

1. **Copy-verify-then-delete — never a bare move.** The archive is copied to the
   share, its SHA-256 verified *there*, and only then is the staged copy removed.
   A move that fails midway across a network boundary can leave zero good copies.
2. **Partial transfers are named `.part`,** so an interrupted copy never matches a
   `*.tar.gz` glob and cannot be mistaken for a finished archive.
3. **A verified archive is never deleted by cleanup.** Once confirmed good at the
   destination, a later-stage failure will not remove it.
4. **The checksum file always points at the archive's current location** — an
   absolute path, so `sha256sum -c` resolves from any working directory. When the
   archive moves, the checksum file is rewritten.
5. **Logs are published on failure too.** They are written locally during the run
   — a log on the share is useless when the share is what failed — and moved to
   `logs/<date>/` on both paths.
6. **The destination is mandatory,** validated at startup and again in pre-flight,
   and it must not live inside the staging area that gets wiped every run.
7. **Nothing unprepared is ever published or started.** Asserted at publish time
   and again against the extracted datadir.
8. **Verify before destroying.** The archive checksum is confirmed before MySQL is
   stopped; binlog checksums before any is applied.
9. **Halt, never limp on.** A failed apply stops at the failed file, a failed
   collection exits non-zero, and partial success is reported as failure.

## 11. Configuration

### Must match across scripts

Get these wrong and the scripts silently stop finding each other. **The first two
failure modes are silent** — after editing any script, verify the set still agrees.
...
| `LOCK_DIR` + `BACKUP_BASE_NAME` | backup, collect | collector never sees the lock → runs *during* a backup |
...
| `CONFIRM_WIPE` | `restore_full.sh` off switch — see §7 |
...
|---|---|
| Collector exits 0, collects nothing, no anchor found | no backup has completed yet — or `SECONDARY_STORAGE_DIR` differs between scripts |
| Collector runs *during* a backup | `LOCK_DIR` or `BACKUP_BASE_NAME` mismatch |
| Collector exits 1, "not mounted" / "not writable" | share is down or the handle is stale — deliberate loud failure |
| `--prepare` fails, or the archive is corrupt at restore | staging is on the CIFS mount; it must be local disk (§1) |
| "Not writable" on the share | the mount's `uid=`/`gid=`/`file_mode=`/`dir_mode=` options |
| "Insufficient space" during backup | staging needs ~1.5x data size; raise `LOCAL_SPACE_PCT` if compression is poor |
| Same-day rerun overwrote a backup | two jobs sharing one destination folder (§3) |
...
| MySQL will not start after a restore | check the version warning in the restore log first (§7) |
...
should not extend the lock window. When writing one, prune a backup and its
`binlog/<date>/` and `logs/<date>/` folders **together**: removing an archive
while leaving its binlogs behind leaves collected logs anchored to a backup that
no longer exists.

**Partial / incremental backups** — every run is a full backup.
**Restoring a single database** — XtraBackup restores the whole instance; use the
[logical chain](../logical/logical-backup.md) for per-database recovery.
