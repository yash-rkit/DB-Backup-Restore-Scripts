...
# Example: ./apply_binlog.sh 20260810 --dry-run
#
set -euo pipefail
...
# EXACTLY six digits: a bare glob also matches binlog.sha256, whose "sha256"
# is read as a variable name inside $(( )) and aborts under set -u.
...
# Only the binlogs and their checksum file; rsync preferred, cp fallback.
STAGE_OK=false
...
# Against the checksums recorded at collection time.
...
PREV_SEQ=""
while read -r f; do
  # Strip leading zeros: bash arithmetic reads 000042 as octal.
...
for BINLOG_FILE in "${APPLY_LIST[@]}"; do
  ...
  if [[ "$BINLOG_FILE" == "$START_BINLOG" ]]; then
    ...
    # --disable-log-bin: replayed events must not re-enter this server's binlog.
    if ! "$MYSQLBINLOG_BIN" \
        --disable-log-bin \
