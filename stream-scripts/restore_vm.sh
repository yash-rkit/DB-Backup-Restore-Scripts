#!/usr/bin/env bash
#
# stream-scripts/restore_vm.sh — argument-driven restore for the restore VM
#
# Same three phases as restore.sh, but the source server is an ARGUMENT, not a
# constant: one invocation restores one production server's archive onto this
# host. final.sh loops the JSON server list and calls this once per entry.
#
# ERASES MYSQL_DATADIR — every invocation, for every server. Take a VM
# snapshot first, and never point this at a production host.
...
#   PART 7   identity, backup id and paths
...
# Per-server, so both arrive as arguments (see PART 5):
#   --server_name   identity only: log names, marker names, the summary
#   --backup_base   the producing host's SECONDARY_STORAGE_DIR on the share
SERVER_NAME=""
SECONDARY_STORAGE_DIR=""                             # set from --backup_base
...
RESTORE_COMPLETE=false   # MySQL is up on the restored datadir
DATADIR_WIPED=false
APPLY_STARTED=false
APPLIED_COUNT=0
LAST_APPLIED=""
...
fail_run() {
  ...
  if [[ "$APPLY_STARTED" == true ]]; then
    ...
    cerr "3. roll back and retry the whole thing:  $SELF_CMD"
  ...
  banner " RESULT failed server=${SERVER_NAME:-none} id=${BACKUP_ID:-none} phase=${at% *} step=${at#* } applied=${APPLIED_COUNT} dur_s=$(( $(date +%s) - START_EPOCH )) warn=${WARN_COUNT}"
...
usage() {
  cat <<EOF
Usage: $0 --server_name=NAME --backup_base=PATH
  ...
  Phase 2 PERMANENTLY ERASES $MYSQL_DATADIR — every run, whichever server is
  named. Only ever run this on a dedicated restore VM.

  --server_name=NAME      source server; used for log, marker and lock names
  --backup_base=PATH      that server's directory on the share: the same path
                          its backup.sh publishes to (its SECONDARY_STORAGE_DIR)
  --backup_id=ID          exact archive: 20260820, or 20260820_143005 for a
                          second run on the same day
  --backup_date=YYYYMMDD  that day's LATEST archive (default: today)
  ...
  --binlog-only           phase 3 only; needs a previous restore of this backup id

Examples:
  B=/livestorage/Backup/Cloud-Live-DB-Default
  $0 --server_name=Cloud-Live-DB-Default --backup_base=\$B --dry-run
  $0 --server_name=Cloud-Live-DB-Default --backup_base=\$B
  $0 --server_name=Cloud-Live-DB-Default --backup_base=\$B --backup_date=20260820
  $0 --server_name=Cloud-Live-DB-Default --backup_base=\$B --backup_id=20260820_143005 --skip-binlog
EOF
  trap - ERR INT TERM
  exit 1
}
...
for arg in "$@"; do
  case "$arg" in
    ...
    --backup_base=*)  SECONDARY_STORAGE_DIR="${arg#*=}" ;;
    --backup_id=*)    BACKUP_ID="${arg#*=}" ;;
    --backup_date=*)  BACKUP_DATE="${arg#*=}" ;;
    --from=*)         FROM_BINLOG="${arg#*=}" ;;
    --dry-run)        DRY_RUN=1 ;;
    --skip-binlog)    SKIP_BINLOG=1 ;;
    --binlog-only)    BINLOG_ONLY=1 ;;
    -h|--help)        usage ;;
...
[[ -n "$SECONDARY_STORAGE_DIR" ]] || argfail "--backup_base is required"
...
[[ "$SECONDARY_STORAGE_DIR" == /* ]] \
  || argfail "--backup_base must be an absolute path: $SECONDARY_STORAGE_DIR"
SECONDARY_STORAGE_DIR="${SECONDARY_STORAGE_DIR%/}"

[[ -z "$BACKUP_ID" || -z "$BACKUP_DATE" ]] \
  || argfail "--backup_id and --backup_date are contradictory — pass one."
...
if [[ -n "$BACKUP_ID" ]] && ! [[ "$BACKUP_ID" =~ ^[0-9]{8}(_[0-9]{6})?$ ]]; then
  argfail "Invalid --backup_id: $BACKUP_ID (expected YYYYMMDD or YYYYMMDD_HHMMSS)"
fi
...
# PART 7  IDENTITY, BACKUP ID AND PATHS
#
# The backup id is resolved BEFORE the log exists, because the log is named
# after it. Failures here therefore print plainly and exit — there is nowhere
# to write a formatted line yet.
# ═══════════════════════════════════════════════════════════════════════════

# Newest published archive for one date. A bare date is treated as time 000000
# so the two naming forms sort together: a plain lexical sort puts
# 20260820_143005 BEFORE 20260820 ('_' sorts before end-of-string) and would
# silently pick the day's FIRST backup. Date and time are separate sort keys.
# Load-bearing — do not reduce it to `sort | tail -1`.
pick_latest_archive() {
  local date="$1" f id d t
  { while IFS= read -r f; do
      id="$(basename "$f" .xbstream)"
      [[ "$id" =~ ^[0-9]{8}(_[0-9]{6})?$ ]] || continue
      d="${id%%_*}"; t="${id#*_}"; [[ "$t" == "$d" ]] && t="000000"
      printf '%s %s %s\n' "$d" "$t" "$id"
    done < <(find "$SECONDARY_STORAGE_DIR" -maxdepth 1 -type f \
               -name "${date}*.xbstream" 2>/dev/null || true)
  } | sort -k1,1 -k2,2 | tail -1 | awk '{print $3}'
  return 0
}

# Same ordering, over this server's restore markers rather than the share:
# --binlog-only needs an id that was restored HERE, and its archive may since
# have been pruned.
pick_latest_marker() {
  local date="$1" f id d t
  { while IFS= read -r f; do
      id="$(basename "$f" _restore_state)"; id="${id#"${SERVER_NAME}_"}"
      [[ "$id" =~ ^[0-9]{8}(_[0-9]{6})?$ ]] || continue
      d="${id%%_*}"; t="${id#*_}"; [[ "$t" == "$d" ]] && t="000000"
      printf '%s %s %s\n' "$d" "$t" "$id"
    done < <(find "$STATE_DIR" -maxdepth 1 -type f \
               -name "${SERVER_NAME}_${date}*_restore_state" 2>/dev/null || true)
  } | sort -k1,1 -k2,2 | tail -1 | awk '{print $3}'
  return 0
}

if [[ -z "$BACKUP_ID" ]]; then
  RESOLVE_DATE="${BACKUP_DATE:-$(date +%Y%m%d)}"

  # The same trap CLAUDE.md describes: an unmounted share is an empty local
  # directory that passes every -d/-r test, so "no archive found" and "the NAS
  # is gone" would otherwise read identically.
  if ! mountpoint -q "$SMB_MOUNT_POINT" 2>/dev/null; then
    echo "[ERROR] $SMB_MOUNT_POINT is not a mount point — the share is not mounted." >&2
    echo "        Cannot pick a backup id for $SERVER_NAME. Mount it and re-run." >&2
    trap - ERR INT TERM; exit 1
  fi
  [[ -d "$SECONDARY_STORAGE_DIR" ]] || {
    echo "[ERROR] --backup_base does not exist on the share: $SECONDARY_STORAGE_DIR" >&2
    trap - ERR INT TERM; exit 1; }

  if [[ $BINLOG_ONLY -eq 1 ]]; then
    BACKUP_ID="$(pick_latest_marker "$RESOLVE_DATE")"
    ID_FROM="latest restore marker for $RESOLVE_DATE"
    [[ -n "$BACKUP_ID" ]] || {
      echo "[ERROR] No restore of $SERVER_NAME on $RESOLVE_DATE is recorded in $STATE_DIR." >&2
      echo "        --binlog-only replays into a datadir this script restored; there is" >&2
      echo "        no record of one. Run the full restore, or pass --backup_id." >&2
      trap - ERR INT TERM; exit 1; }
  else
    BACKUP_ID="$(pick_latest_archive "$RESOLVE_DATE")"
    ID_FROM="latest archive for $RESOLVE_DATE"
    [[ -n "$BACKUP_ID" ]] || {
      echo "[ERROR] No ${RESOLVE_DATE}*.xbstream in $SECONDARY_STORAGE_DIR" >&2
      echo "        (server $SERVER_NAME). Available:" >&2
      find "$SECONDARY_STORAGE_DIR" -maxdepth 1 -type f -name '*.xbstream' 2>/dev/null \
        | sort | while read -r f; do echo "          $(basename "$f" .xbstream)" >&2; done
      trap - ERR INT TERM; exit 1; }
  fi
else
  ID_FROM="--backup_id"
...
# The local copy the restore actually reads. RESTORE_SOURCE is set in the verify
# phase to whichever of the two the extract will consume.
...
# Keyed by SERVER as well as id. Every server's daily archive is called
# 20260821, so a marker keyed on the id alone would have the second server of
# the night reading the first one's binlog anchor — the wrong start position,
# applied silently.
RESTORE_MARKER="${STATE_DIR}/${SERVER_NAME}_${BACKUP_ID}_restore_state"
LOCAL_BINLOG_DIR="${LOCAL_STAGE}/binlog_${SERVER_NAME}_${BACKUP_ID}"

RUN_STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_LOG="${LOCAL_STAGE}/${SERVER_NAME}_${BACKUP_ID}_restore_${RUN_STAMP}.log"
ERROR_LOG="${LOCAL_STAGE}/${SERVER_NAME}_${BACKUP_ID}_restore_${RUN_STAMP}_errors.log"
XTRABACKUP_LOG="${LOCAL_STAGE}/${SERVER_NAME}_${BACKUP_ID}_restore_${RUN_STAMP}_xtrabackup.log"
SECONDARY_LOG_DIR="${SECONDARY_STORAGE_DIR}/logs/${BACKUP_ID}/restore_${RUN_STAMP}"

# Printed by every failure path and by the summary, so the retry command is
# always the full one — no argument left for the operator to remember.
SELF_CMD="$0 --server_name=$SERVER_NAME --backup_base=$SECONDARY_STORAGE_DIR --backup_id=$BACKUP_ID"

mkdir -p "$LOCAL_STAGE" 2>/dev/null || {
  echo "[ERROR] Failed to create $LOCAL_STAGE" >&2
  trap - ERR INT TERM; exit 1; }

printf 'errors for restore of %s %s (attempt %s)\n\n' "$SERVER_NAME" "$BACKUP_ID" "$RUN_STAMP" > "$ERROR_LOG"

banner " RESTORE RUN  $SERVER_NAME  $BACKUP_ID"
...
# --backup_base is an argument, so a typo is a plausible daily event, and a
# path OUTSIDE the mount reads as a valid empty directory on the root
# filesystem: 'no archives found' rather than 'wrong path'. mountpoint is only
# ever true for the mount point itself, never a subdirectory, so the check has
# to be a prefix test.
check
[[ "$SECONDARY_STORAGE_DIR" == "$SMB_MOUNT_POINT"/* ]] \
  || die "$(leader 'backup base' 'OFF THE SHARE')" \
         "--backup_base is $SECONDARY_STORAGE_DIR" \
...
if [[ $DRY_RUN -eq 1 ]]; then
  ...
  if [[ $DO_APPLY -eq 1 ]]; then
    ...
    PREVIEW_FILES=${#PATHS[@]}
    PREVIEW="${LOCAL_STAGE}/${SERVER_NAME}_${BACKUP_ID}_binlog_preview.sql"
  ...
  emit ""
  banner " DRY RUN COMPLETE — NOTHING WAS MODIFIED  ($SERVER_NAME)"
  ...
  if [[ "$SHA_RESULT" == "MISMATCH" ]]; then
    banner " RESULT failed server=${SERVER_NAME} id=${BACKUP_ID} phase=dryrun reason=checksum dur_s=$(( $(date +%s) - START_EPOCH ))"
    publish_logs
    trap - ERR INT TERM
    exit 1
  fi
  banner " RESULT ok server=${SERVER_NAME} id=${BACKUP_ID} phase=dryrun files=${PREVIEW_FILES} dur_s=$(( $(date +%s) - START_EPOCH )) warn=${WARN_COUNT}"
  publish_logs
  trap - ERR INT TERM
  exit 0
fi
...
# PART 12  RESTORE  2/3
#
# Extract, decompress and prepare all happen IN PLACE in the datadir: no
# --copy-back, so ~1x the data size there rather than 2x. With STAGE_ARCHIVE=1
# the compressed archive is additionally held on local disk (see PART 11), which
# the "archive staging space" pre-flight accounts for. Everything from
...
# it would silently hold 14-30GB per server until someone noticed.
...
if [[ $DO_APPLY -eq 0 ]]; then
  emit "   2. binlogs are NOT applied — this copy stops at the backup point."
  emit "        $SELF_CMD --binlog-only --dry-run"
  emit "        $SELF_CMD --binlog-only"
  emit "   3. then dump it:  ./logical.sh --server_name=$SERVER_NAME --base_dir=<dumps>/$SERVER_NAME"
else
  emit "   2. dump it:  ./logical.sh --server_name=$SERVER_NAME --base_dir=<dumps>/$SERVER_NAME"
fi
emit "      this datadir is TEMPORARY: the next restore_vm.sh run on this host"
emit "      erases it, whichever server that run is for. The logical dump is the"
emit "      only lasting product of the restore."
sub
kv "logs" "$SECONDARY_LOG_DIR/"
banner " RESULT ok server=${SERVER_NAME} id=${BACKUP_ID} applied=${APPLIED_COUNT} gaps=${GAPS} dur_s=$(( $(date +%s) - START_EPOCH )) warn=${WARN_COUNT}"
