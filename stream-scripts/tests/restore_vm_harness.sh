#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════
# restore_vm.sh test harness
#
# Runs restore_vm.sh end to end against stub binaries and a fake share, so the
...
#
#   ./tests/restore_vm_harness.sh          # all scenarios
#   ./tests/restore_vm_harness.sh 3        # one scenario by number
#   KEEP=1 ./tests/restore_vm_harness.sh   # keep the sandbox for inspection
#
...
mkdir -p "$H"/bin "$H"/state "$H"/livestorage/YK/Restore-VM/binlog/20260821 \
         ...
         "$H"/var/lock/dbvault "$H"/var/log/mysql
cd "$H"
...
build() {
  sed -e "s|^SMB_MOUNT_POINT=.*|SMB_MOUNT_POINT=\"$H/livestorage\"|" \
      ...
      "$TARGET" > "$H/rv.sh"
  bash -n "$H/rv.sh" || { echo "SYNTAX ERROR in the generated copy"; exit 1; }
}
fresh() { rm -rf Data/mysql; rm -f var/lib/dbvault/*; mkdir -p Data/mysql
          echo junk > Data/mysql/old.ibd; echo running > state/mysqld; }
run()   { bash "$H/rv.sh" --server_name=Restore-VM --backup_base="$BASE" \
                          --backup_id=20260821 "$@" 2>&1; }
...
if want 8; then head_ "8  STAGE_ARCHIVE=0 keeps the old off-the-share path"
  ...
  expect "extracts from the share as before" 'staging space .*STAGE_ARCHIVE=0' \
         'reading the whole archive off the share' 'RESULT ok '
fi

if want 9; then head_ "9  a stage dir inside the datadir is refused at pre-flight"
  STAGE=1 build; fresh
  sed -i "s|^ARCHIVE_STAGE_DIR=.*|ARCHIVE_STAGE_DIR=\"$H/Data/mysql/stage\"|" "$H/rv.sh"
  OUT="$(STUB_PING=alive run)"
...
if want 11; then head_ "11  rejected credentials recover via --binlog-only"
  ...
  expect "step 2 applies the binlogs alone" 'binlogs_applied=no' \
         'applied 2 file' 'RESULT ok '
fi
