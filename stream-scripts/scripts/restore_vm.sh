...
SECONDARY_STORAGE_DIR=""                             # --backup_base=, the source server's dir on the share
...
SET_ME_VARS=(MYSQL_USER MYSQL_PASSWORD SMB_MOUNT_POINT)
...
mountpoint -q "$BASE_MOUNT" \
  || die "$(leader 'backup base' 'NOT MOUNTED')" \
         "$BASE_MOUNT holds --backup_base but is not a mount point"
