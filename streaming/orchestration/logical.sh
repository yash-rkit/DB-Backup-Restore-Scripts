#!/usr/bin/env bash
#
# Usage:   ./logical.sh --server_name=NAME --base_dir=PATH [options]
# Example: ./logical.sh --server_name=db01 --base_dir=/livestorage/Logical/db01
#
set -euo pipefail
...
# PART 1E, deferred: the credential override above has now had its chance.
check_set_me
...
# Per-database status files: an xargs worker runs in a subshell, so its variables die with it.
...
# `|| true` on every list build: a grep matching nothing exits 1, and pipefail would fail the assignment.
...
dump_one() {
  ...
  # Read it back — local, so this costs nothing.
