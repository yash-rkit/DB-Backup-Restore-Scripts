#!/usr/bin/env bash
#
# Usage:   ./restore_vm.sh --server_name=NAME --backup_base=PATH [options]
# Example: ./restore_vm.sh --server_name=db01 --backup_base=/livestorage/db01 --dry-run
#
set -euo pipefail
...
# ───────────────────────────────────────────────────────────────────────────
# PART 1D resolution. Order matters: mysql_up needs the service name, and resolve_datadir consults it.
...
if [[ $DO_RESTORE -eq 0 ]]; then
  ...
  [[ -z "$M_FORMAT" || "$M_FORMAT" == "xbstream" ]] \
    || die "$(leader 'manifest' "format=$M_FORMAT")" \
           ...
           "for a .tar.gz backup use standalone/physical/restore_full.sh"
  [[ "$M_PREPARED" != "yes" ]] \
...
if [[ $DO_RESTORE -eq 0 ]]; then
  ...
  DATADIR_WIPED=true
  # find, not a glob: a glob misses dotfiles and stays unexpanded on an empty datadir.
  ...
  # `|| true`: find gets SIGPIPE from head, which pipefail would turn into a failure.
  ...
  # Wait-loop outcomes: 2002 keep waiting, a crash stops now, 1045 means READY
  # (a restore overwrites mysql.user with the SOURCE accounts).
  WAIT_START=$(date +%s)
