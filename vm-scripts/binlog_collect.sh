...
# Remote / secondary storage directory (matches backup.sh)
...
############################
# LOCK FILE CHECK
# First thing we do — before any initialization.
...
LOCK_FILE="${ARCHIVE_DIR}/${TODAY_LOCK}_lock"
...
if [[ -f "${ARCHIVE_DIR}/${TODAY}_binlog_info" ]]; then
  BINLOG_INFO_FILE="${ARCHIVE_DIR}/${TODAY}_binlog_info"
  BINLOG_INFO_SOURCE="today"
elif [[ -f "${ARCHIVE_DIR}/${YESTERDAY}_binlog_info" ]]; then
  BINLOG_INFO_FILE="${ARCHIVE_DIR}/${YESTERDAY}_binlog_info"
  BINLOG_INFO_SOURCE="yesterday"
else
  # No binlog_info means backup has never completed yet — exit cleanly
  echo "[$(date '+%F %T')] [INFO] No binlog_info file found for today ($TODAY) or yesterday ($YESTERDAY)." >&2
