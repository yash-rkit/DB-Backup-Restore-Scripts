...
# Set via --server_name / --backup_base (see INPUT VALIDATION); ARCHIVE_DIR and
# LOCK_FILE are derived from BACKUP_BASE once it is known
SERVER_NAME=""
BACKUP_BASE=""
...
# Show usage
show_usage() {
  echo "Usage: $0 --server_name=NAME --backup_base=PATH [--backup_date=YYYYMMDD]"
  echo ""
  echo "Arguments:"
  echo "  --server_name=NAME      :  Identifier for the source server (used in logs)"
  echo "  --backup_base=PATH      :  Root path of this server's Percona backup archive"
  echo "  --backup_date=YYYYMMDD  :  Date of backup to restore (optional, defaults to today)"
  echo ""
  echo "Examples:"
  echo "  $0 --server_name=LiveDB-2 --backup_base=/livestorage/DBBackup/LiveDB-2/percona"
  echo "  $0 --server_name=LiveDB-2 --backup_base=/livestorage/DBBackup/LiveDB-2/percona --backup_date=20260106"
...
if [[ -z "$SERVER_NAME" || -z "$BACKUP_BASE" ]]; then
  echo "[$(date '+%F %T')] [ERROR] --server_name and --backup_base are required" >&2
  show_usage
fi
