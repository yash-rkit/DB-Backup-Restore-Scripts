#!/usr/bin/env bash
#
# db-vault/physical/smb/backup.sh — portal (DB Vault) version, SMB/CIFS storage
#
# SMB/CIFS variant. The WORK MODEL is unchanged from physical_backup.sh:
# xtrabackup writes to LOCAL staging, ownership is set there, --prepare and
# compression run there, and the finished archive is then MOVED to the SMB
# share. (Local staging is required on CIFS anyway: --prepare needs POSIX
# ownership, byte-range locking and atomic renames, which CIFS cannot provide.)
#
# What differs from the original:
#   - archives are named by date alone (<date>.tar.gz), no job<CONFIG_ID>_ prefix
#   - CONFIG_ID is read from $8 (was $9 — see the 2026-07-21 arg-contract change)
#   - logs are published to ARCHIVE_DIR/logs/<date>/ instead of sitting loose
#     in the archive root
#
...
#
# Parameterization (per DB Vault Script Authoring Guide):
...
#   the API passes positional args $1-$8; this script uses $1, $2, $5, $7 and $8.
#
...
BASE_DIR="$5"
STATUS_DB="$7"
CONFIG_ID="${8:-}"         # BJC03.ConfigId — the backup job *definition* this run
...
CREATE TABLE IF NOT EXISTS ${STATUS_DB}.BLS04 (
    ...
    S04F05 VARCHAR(64) NULL
        COMMENT 'ReferenceAnchor (<date>, ties collection runs to a backup)',
...
# Move this run's logs from local staging to ARCHIVE_DIR/logs/<NAME>/, dropping
# the redundant date prefix (the folder already carries it). Called on BOTH the
# success and failure paths so staging is always left clean and every run is
# debuggable from one place.
#
# Entirely best-effort: never fails the run, and never routes through
# fail_backup (which calls it — that would recurse). Uses plain echo rather
# than log_msg, because after this runs RUN_LOG no longer exists on staging.
PUBLISHED_LOGS=0
publish_logs() {
  [[ "$PUBLISHED_LOGS" == "1" ]] && return 0     # idempotent; only ever once
  PUBLISHED_LOGS=1
  [[ -n "${ARCHIVE_LOG_DIR:-}" ]] || return 0

  mkdir -p "$ARCHIVE_LOG_DIR" 2>/dev/null || {
    echo "[$(date '+%F %T')] [WARN] Cannot create log dir: $ARCHIVE_LOG_DIR (logs remain in staging)" >&2
    return 0
  }

  local src dst
  # staging file -> published name (date prefix dropped)
  for pair in "${RUN_LOG}:backup.log" \
              "${XTRABACKUP_LOG}:xtrabackup.log" \
              "${ERROR_LOG}:errors.log"; do
    src="${pair%:*}"; dst="${pair##*:}"
    [[ -n "$src" && -f "$src" ]] || continue
    # `cp` then remove rather than `mv`: staging and the share are different
    # filesystems, and on CIFS a cross-device mv can fail partway through
    # trying to preserve attributes the share cannot represent.
    if cp "$src" "${ARCHIVE_LOG_DIR}/${dst}" 2>/dev/null; then
      rm -f "$src" 2>/dev/null || true
    fi
  done

  echo "[$(date '+%F %T')] [INFO] Logs published to: $ARCHIVE_LOG_DIR"
}
...
fail_backup() {
  ...
  log_error "Backup aborted. Check logs: ${ARCHIVE_LOG_DIR:-${RUN_LOG:-'(not initialized)'}}"

  # Publish this failed run's logs to the share before staging is abandoned —
  # a failure is exactly when the logs matter most. Done last so it captures
  # every cleanup message above.
  publish_logs
  exit 1
}
...
# Base name for every artifact of this run: the run date only.
#   e.g. 20260711.tar.gz / 20260711.sha256 / 20260711_binlog_info
# On a same-day collision a time component is appended below: 20260711_143005.
#
# NOTE: deliberately NOT namespaced with job<CONFIG_ID>. One ARCHIVE_FOLDER
# holds exactly one backup job's output, so the folder already provides that
# scoping and the job id in the filename was redundant. CONFIG_ID is still
# recorded in BLS02.S02F19 / BLS04.S04F03 for the dashboard.
#
# Consequence: two jobs must never share an ARCHIVE_FOLDER — without the job
# prefix their same-day archives would collide on identical names.
NAME="${TODAY}"
...
# --- Log layout -----------------------------------------------------------
# Logs are WRITTEN to local staging during the run, then MOVED to the share at
# the end — on success AND on failure, so every run is debuggable from one
# place. Final resting place, one folder per run, kept OUT of the archive root
# so retention can prune logs independently and `*.tar.gz` globs stay clean:
#
#   ARCHIVE_DIR/logs/<NAME>/backup.log
#                          /xtrabackup.log
#                          /errors.log
#                          /collect/…      (written by binlog_collect.sh)
ARCHIVE_LOG_DIR="${ARCHIVE_DIR}/logs/${NAME}"

# Staging log paths used while the run is in progress.
...
# exists, to keep the name unique): 20260711_143005.
...
# the already-archived <date>.tar.gz and could overwrite it on move.
if [[ -d "$TARGET_DIR" || -f "$COMPRESSED_FILE" || -f "${ARCHIVE_DIR}/${NAME}.tar.gz" ]]; then
  ...
  RUN_LOG="${BACKUP_BASE}/${NAME}_backup.log"
  ERROR_LOG="${BACKUP_BASE}/${NAME}_errors.log"
  # Must be re-derived too, or this rerun's logs would land in the earlier
  # run's folder on the share.
  ARCHIVE_LOG_DIR="${ARCHIVE_DIR}/logs/${NAME}"
fi
...

{
  ...
  echo "===================================================="
  echo "Logs (published to the share):"
  echo "  $ARCHIVE_LOG_DIR/backup.log"
  echo "  $ARCHIVE_LOG_DIR/xtrabackup.log"
  echo "  $ARCHIVE_LOG_DIR/errors.log"
  echo "===================================================="
} | tee -a "$RUN_LOG"
...
############################
# PUBLISH LOGS
# Last action: moves the run's logs off local staging into
# ARCHIVE_DIR/logs/<NAME>/. Nothing may log via log_msg after this point —
# RUN_LOG no longer exists in staging.
############################
