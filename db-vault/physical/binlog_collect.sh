#!/usr/bin/env bash
#
# db-vault/physical/smb/binlog_collect.sh — portal (DB Vault) version, SMB/CIFS storage
#
# SMB/CIFS variant. Collection logic is unchanged from binlog_collect.sh.
# What differs:
#   - the anchor is picked by filename (date), not mtime — SMB mtime comes from
#     the server clock and is attribute-cached, so it is not trustworthy
#   - `cp` instead of `cp -p` (CIFS cannot preserve ownership; -p makes cp
#     return non-zero on otherwise-good copies)
#   - logs go to ARCHIVE_DIR/logs/<anchor>/collect/, not beside the binlogs
#
...
# as a scheduled DB Vault job; each run resumes from where the last one left off
...
CREATE TABLE IF NOT EXISTS ${STATUS_DB}.BLS04 (
    ...
    S04F05 VARCHAR(64) NULL
        COMMENT 'ReferenceAnchor (<date>, ties runs to a backup)',
...
# ARCHIVE_FOLDER is scoped to a single backup job's output, so the latest
# *_binlog_info in ARCHIVE_DIR is this job's most recent backup. Its filename
# (<date>, e.g. 20260711) is recorded as the reference so runs group under it.
#
# Selected by FILENAME, not mtime. On CIFS/SMB, mtime comes from the server's
# clock and is subject to attribute caching (actimeo) and DST/timezone
# handling, so "newest by mtime" can pick the wrong anchor after a clock skew —
# which would silently reset collection to an older backup and break PITR
# coverage.
#
# A plain lexical sort is NOT enough: a same-day rerun is named
# 20260711_143005, and '_' sorts before end-of-string, so 20260711_143005 would
# come BEFORE 20260711 and the rerun would be ignored. Sorting the date and the
# (possibly absent) time as separate keys fixes that — a bare date is treated
# as time 000000, so any same-day rerun correctly wins.
############################
...
# Logs live under logs/<anchor>/collect/, NOT beside the binlogs. Keeps
# binlog/<anchor>/ pure restorable data (binlogs + resume state) and puts every
# log for a given backup — the backup's own plus all its collection runs — in
# one place. Appended across runs, since this fires every ~15 minutes.
COLLECT_LOG_DIR="${ARCHIVE_DIR}/logs/${ANCHOR_BASE}/collect"
...
if ! mkdir -p "$COLLECT_LOG_DIR" 2>/dev/null; then
  log_error "Failed to create log directory: $COLLECT_LOG_DIR"
  fail_collect 1
fi
...
for BINLOG_FILE in "${BINLOG_FILES[@]}"; do
  ...
  # `cp` without -p: on CIFS, preserving ownership/mode fails (the server owns
  # the files; the client's view is fixed by the mount's uid=/gid=), and cp
  # returns non-zero even though the DATA copied fine — which would wrongly
  # count good binlogs as errors. The size check below is the real integrity
  # guarantee, and mtime is not relied on for anything here.
  if ! cp "$local_src" "$TARGET_BINLOG_DIR/" 2>>"$ERROR_LOG"; then
